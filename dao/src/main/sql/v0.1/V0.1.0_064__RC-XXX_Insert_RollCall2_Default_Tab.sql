DECLARE
    ...
    BEGIN
		...
		BEGIN
		  ...
		  from DBO_MP.LOOKUP_TYPE SP
		, DBO_MP.LOOKUP SPN
		WHERE LOOKUP_TYPE_NAME='DEFAULT_TAB'
		AND SPN.LOOKUP_TYPE_ID=SP.LOOKUP_TYPE_ID
		GROUP BY SP.LOOKUP_TYPE_ID;
		  EXCEPTION
			WHEN OTHERS  THEN 
			  DBMS_OUTPUT.PUT_LINE('LOOKUP for does not exist DEFAULT_TAB');
			END;
			--
		...
		BEGIN
			...
			V_LOOKUP_EXISTS:=LOOKUP_VALUE_FNC('RollCall2','DEFAULT_TAB');
			IF V_LOOKUP_EXISTS='N' THEN 
				LOOKUP_VALUE_INS('RollCall2',V_LOOKUP_TYPE_ID,V_SORT_ORDER);
			END IF;
			END;
	--
	  COMMIT;
